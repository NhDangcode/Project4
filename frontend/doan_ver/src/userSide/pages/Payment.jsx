...
const Payment = ({ open, onSetOpen }) => {
    ...
    const onSuccess = async (type) => {
        const result = await createOrderService(type === 0 ? 1 : 2, isTransport ? 1 : 0);
    ...
    const handleGetData = async () => {
        ...
        result.status === 200 ? setPrice((result.total / 23507).toFixed()) : setPrice();
    }
    useEffect(() => {
        handleGetData();
    }, []);
    return (
        <>
            <Modal
                ...
                title="THANH TOÁN HOÁ ĐƠN" open={open}>
                <Card title="HÌNH THỨC VẬN CHUYỂN">
                    <Radio.Group onChange={onChange} value={isTransport}>
                        ...
                        <p style={{ marginBottom: "10px" }}>Chúng tôi sẽ liên hệ bạn sớm nhất trong thời gian có thể. Đơn hàng của bạn sẽ được vận chuyển trong thời gian 2-3 ngày làm việc.</p>
                ...
                <Card title="HÌNH THỨC THANH TOÁN" style={{ marginTop: "20px" }}>
                    <Radio.Group onChange={onChangePayment} value={isPayment}>
                        <Radio style={{ marginBottom: "10px" }} value={true}>Thanh toán bằng thẻ ngân hàng</Radio>
                        <Radio value={false}>Sau khi nhận được hàng</Radio>
                        <p>Nhân viên sẽ liên lạc với bạn và giao hàng. Bạn sẽ trực tiếp thanh toán với nhân viên giao hàng.</p>
                    </Radio.Group>
                </Card>
                <Row style={{ marginTop: "20px" }}>
                    <Col span={10}>
                        <Button block type="primary" ghost onClick={onCancel}>CANCEL</Button>
                    </Col>
                    <Col span={10} offset={4}>
                        {
                            isPayment ? <PayPalScriptProvider options={{ clientId: "test", currency: "", }}>
                                <PayPalButtons
                                ...
                                />
                            </PayPalScriptProvider> : <Button block type="primary" onClick={() => onSuccess(0)}>THANH TOÁN</Button>
